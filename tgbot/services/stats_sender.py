...
async def send_stats(bot: Bot, day: bool, repo: Repo):  # @TODO рассылка всем партнерам
    config = load_config("bot.ini")
    for partner in await repo.get_partners(with_main=True):
        if day:
            orders = await repo.get_orders_count(city=partner['city'], date_range="day")
            if len(orders) > 0:
                ...
                if len(couriers_stats) > 0:
                    ...
                    courier_of_the_day = await repo.get_courier(userid=first_courier_from_list)
...
Статистика курьеров:
        """
                    ...
                    for courier in couriers_stats:
                        courier_data = await repo.get_courier(userid=courier)
...
<code>Нет статистики по курьерам</code>"""
 ...
 📦 Всего заказов за день: Заказов не было"""
            ...
            orders = await repo.get_orders_count(city=partner['city'], date_range="week")
            if len(orders) > 0:
                day_count = orders[0]['count']
                ...
                if len(couriers_stats) > 0:
                    sorted(couriers_stats.items(), key=lambda x: x[1], reverse=True)
                    first_courier_from_list = next(iter(couriers_stats))
                    courier_of_the_week = await repo.get_courier(userid=first_courier_from_list)
